...
tasks. PDFs and EPUBs must remain local-only for now.
 ...
 ```json
 {
  "job_type": "pdf_to_md | epub_to_md | tts_eval | e2e_test",
  "input_path": "/local/path/input.pdf",
  "output_path": "/local/path/job-outputs/",
   "constraints": {
     "max_cost_usd": 5.0,
...
- Storage: local filesystem for artifacts and logs.
...
- Agents can chain runs, e.g. MD cleanup eval -> re-run with different settings.
